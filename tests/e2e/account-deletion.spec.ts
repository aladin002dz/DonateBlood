...
test.describe('Account Deletion Flow', () => {
  ...
  test.describe('Authenticated Account Deletion', () => {
    ...
    test('should display warning information in dialog', async ({ page }) => {
    ...
    test('should require confirmation text to enable delete button', async ({ page }) => {
    ...
    test('should show loading state while deleting', async ({ page }) => {
      // Note: Actual deletion is prevented by PLAYWRIGHT_TEST_MODE env var
      // See playwright.config.ts and actions/delete-account.ts
    ...
    test('should redirect to home after successful deletion', async ({ page }) => {
      // Note: Actual deletion is prevented by PLAYWRIGHT_TEST_MODE env var
      // See playwright.config.ts and actions/delete-account.ts
      ...
      // Note: Since we're using test mode, the session remains valid
      // The user isn't actually deleted, so we can't test session invalidation here
    });
    ...
    test('should show success message after deletion', async ({ page }) => {
      ...
      // Look for success toast/message - increased timeout
      await expect(page.locator('text=/success|deleted|removed/i')).toBeVisible({ timeout: 15000 });
    });
